...
class Action
{
	...
	function save_employee_attendance()
	{
		...
		foreach ($employee_id as $k => $v) {
			$datetime_log[$k] = date("Y-m-d H:i", strtotime($datetime_log[$k]));  // Format konsisten
			$data = " employee_id='$employee_id[$k]' ";
			$data .= ", log_type = '$log_type[$k]' ";
			$data .= ", datetime_log = '$datetime_log[$k]' ";

			// Execute query and check for success
			if (!$this->db->query("INSERT INTO attendance set " . $data)) {
				$errors[] = "Failed to insert attendance record for employee_id $employee_id[$k] at $datetime_log[$k]";
			} else {
				$save[] = true;
			}
		}

		if (empty($errors) && isset($save)) {
			return 1;
		} else {
			// Handle errors (log them, notify user, etc.)
	...
	function delete_employee_attendance()
	{
		extract($_POST);
		$date = explode('_', $id);
		$dt = date("d-m-Y", strtotime($date[1]));

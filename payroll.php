	...
	<div class="col-lg-12">
		...
		<div class="card">
			<div class="card-header">
				<span><b>List Penggajian</b></span>
				<button class="btn btn-primary btn-sm btn-block col-md-3 float-right" type="button" id="new_payroll_btn"><span class="fa fa-plus"></span>Tambah Penggajian</button>
			</div>
			<div class="card-body">
					...
					<tbody>
						...
						?>
							<tr>
								...
								<?php if ($row['status'] == 0) : ?>
									<td class="text-center"><span class="badge badge-primary">Baru</span></td>
								<?php else : ?>
									<td class="text-center"><span class="badge badge-success">Dihitung</span></td>
								<?php endif ?>
								<td>
									<center>
										<?php if ($row['status'] == 0) : ?>
											<button class="btn btn-sm btn-outline-primary calculate_payroll" data-id="<?php echo $row['id'] ?>" type="button">Dihitung</button>
										<?php else : ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.edit_payroll').click(function() {
			...
			uni_modal("Edit Penggajian", "manage_payroll.php?id=" + $id)
		...
		$('#new_payroll_btn').click(function() {
			uni_modal("Penggajian baru", "manage_payroll.php")
		})
		$('.remove_payroll').click(function() {
			_conf("Apakah anda yakin ingin menghapusnya ?", "remove_payroll", [$(this).attr('data-id')])
		})
	...
	function remove_payroll(id) {
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Data gaji karyawan berhasil dihapus", "success");
					setTimeout(function() {
						location.reload();
